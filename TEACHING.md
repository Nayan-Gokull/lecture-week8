...
This is the last thing you say before the module moves on, and it is doing real work: it is the moment eleven separate decisions become one story, and it is the closing statement for the whole AR unit, not just for this one exercise. Give it room — this runs to roughly **six to seven minutes** read at a natural pace, including the two hands-up moments, so budget for it rather than squeezing it into the last ninety seconds of the slot. Every line below is real spoken text except what's inside a bold `[stage direction]` tag — those are never said aloud, and the two `[ask]` moments both have a scripted line that works regardless of what the room actually says back, so you can read this straight through start to finish without needing to improvise anything, other than the optional Discuss/Features quote in thread 6 if you have the board open.
...
<p><b class="sd">[thread 1 — licensing has a price tag, not just a legal status]</b> Start with Source. Every room had the same three-way choice: safe and heavy, cheap and risky, or safe and middling. <b class="sd">[ask, hands up]</b> Who took the cheap one? — <b class="sd">[count the hands, say the number, then continue regardless of what it was]</b> Doesn't matter how many. Here is the thing I want you to actually remember, past this module, past this degree: <b>the cheapest, best-optimised asset on a marketplace is very often the one with the strictest licence.</b> That is not this exercise being artificial. That is why the good stuff costs more or comes with more conditions — someone already did the work you didn't have to. You inspected a real file to make that call, not a paragraph describing one. That is the habit. Read the file. Not the listing.</p>
